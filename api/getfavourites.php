...
mysql_query("SET NAMES 'utf8';");
  ...
  if(mysql_num_rows($result1)) {
   ...
   $query = "SELECT story.imgurl, story.urlid, story.description, story.category, story.distance, story.venue, story.viewed, story.liked, story.lat, story.lng, story.id, user.username, user.imgname FROM story INNER JOIN user INNER JOIN favourites ON story.user_id = user.id AND story.id = favourites.story_id WHERE favourites.email =  '$email' order by favourites.id DESC";
